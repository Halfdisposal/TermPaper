...
\usepackage{mathtools}    % Extends amsmath with useful fixes
\usepackage{geometry}     % Clean page margins
\geometry{margin=1in}
\usepackage{hyperref}     % Optional: improves PDF navigation

\begin{document}
\section*{Coherent States}

\subsection*{Fock representation of the coherent state}
...
\[ \sum_{n=1}^{\infty}c_{n}\sqrt{n}|n-1\rangle=\alpha\sum_{n=0}^{\infty}c_{n}|n\rangle. \]
...
\[ c_{n}=\frac{\alpha^{2}}{[n(n-1)]^{1/2}}c_{n-2}=...=\frac{\alpha^{n}}{\sqrt{n!}}c_{0}, \]
so that
...
and
\[ \langle\alpha|=e^{-|\alpha|^{2}/2}\sum_{n=0}^{\infty}\frac{\alpha^{*n}}{\sqrt{n!}}\langle n|. \]
...
\[ |\alpha\rangle=e^{-|\alpha|^{2}/2}e^{\alpha\hat{a}^{\dagger}}e^{-\alpha^{*}\hat{a}}|vac\rangle. \]
...
We now make use of the Campbell-Baker-Hausdorff operator identity (see Section 10.11) for two operators $\hat{A}$, $\hat{B}$ provided that
\[ \exp(\hat{A}+\hat{B})=\exp\hat{A}\exp\hat{B}\exp(-[\hat{A},\hat{B}]/2), \]
...
\[ e^{\alpha\hat{a}^{\dagger}-\alpha^{*}\hat{a}}=e^{\alpha\hat{a}^{\dagger}}e^{-\alpha^{*}\hat{a}}e^{-|\alpha|^{2}/2} \]
...
\[ \hat{D}^{\dagger}(\alpha)\hat{D}(\alpha)=1=\hat{D}(\alpha)\hat{D}^{\dagger}(\alpha), \]
and
...
{flagged)Unitary transformation of $\hat{a}$ or $\hat{a}^{\dagger}$ leads to the complex displacement $\alpha$ or $\alpha^{*}$, respectively. Thus
\[ \hat{D}^{\dagger}(\alpha)\hat{a}\hat{D}(\alpha)=\hat{a}+\alpha \]
and
\[ \hat{D}^{\dagger}(\alpha)\hat{a}^{\dagger}\hat{D}(\alpha)=\hat{a}^{\dagger}+\alpha^{*}. \]

These properties follow immediately by use of the operator expansion theorem, when we observe that the commutators
\[ [-\alpha\hat{a}^{\dagger}+\alpha^{*}\hat{a},\hat{a}]=\alpha \]
\[ [-\alpha\hat{a}^{\dagger}+\alpha^{*}\hat{a},\hat{a}^{\dagger}]=\alpha^{*} \]
reduce to c-numbers. We then have
\[ e^{-\alpha\hat{a}^{\dagger}+\alpha^{*}\hat{a}}\hat{a}e^{\alpha\hat{a}^{\dagger}-\alpha^{*}\hat{a}}=\hat{a}+\alpha. \]

\section{Squeeze Operator}

\subsection*{The unitary squeeze operator}
...
\subsection*{1. Defining the Operators}
The squeezing operator for a single boson is defined using the squeezing parameter $\xi$:
\begin{equation*}
\hat{S}(\xi) = \exp\left(\frac{1}{2}\xi(\hat{a}^\dagger)^2 - \frac{1}{2}\xi^*\hat{a}^2\right)
\end{equation*}
...
\begin{equation*}
\hat{G} = \frac{1}{2}\xi(\hat{a}^\dagger)^2 - \frac{1}{2}\xi^*\hat{a}^2
\end{equation*}
...
\subsection*{2. Applying the Baker-Campbell-Hausdorff Lemma}
...
\subsection*{3. Calculating the Nested Commutators}
We systematically calculate these individual commutators using the canonical commutation relation $[\hat{a}, \hat{a}^\dagger] = 1$.

\textbf{First Commutator:}
\begin{align*}
[\hat{G}, \hat{a}] &= \left[ \frac{1}{2}\xi(\hat{a}^\dagger)^2 - \frac{1}{2}\xi^*\hat{a}^2, \hat{a} \right] \\
&= \frac{1}{2}\xi [(\hat{a}^\dagger)^2, \hat{a}] - \frac{1}{2}\xi^* [\hat{a}^2, \hat{a}]
\end{align*}
Because any operator commutes with itself, $[\hat{a}^2, \hat{a}] = 0$. Using the identity $[\hat{A}\hat{B}, \hat{C}] = \hat{A}[\hat{B}, \hat{C}] + [\hat{A}, \hat{C}]\hat{B}$, we expand the remaining term:
\begin{align*}
...
\end{align*}
Substituting this result back yields:
\begin{equation*}
...
[\hat{G}, \hat{a}^\dagger] &= \left[ \frac{1}{2}\xi(\hat{a}^\dagger)^2 - \frac{1}{2}\xi^*\hat{a}^2, \hat{a}^\dagger \right] = -\frac{1}{2}\xi^*[\hat{a}^2, \hat{a}^\dagger]
\end{align*}
Expanding $[\hat{a}^2, \hat{a}^\dagger]$:
\begin{equation*}
[\hat{a}^2, \hat{a}^\dagger] = \hat{a}[\hat{a}, \hat{a}^\dagger] + [\hat{a}, \hat{a}^\dagger]\hat{a} = \hat{a}(1) + (1)\hat{a} = 2\hat{a}
\end{equation*}
Evaluating the full bracket gives:
\begin{equation*}
[\hat{G}, \hat{a}^\dagger] = -\frac{1}{2}\xi^*(2\hat{a}) = -\xi^*\hat{a}
\end{equation*}
Returning to the nested commutator:
\begin{equation*}
[\hat{G}, [\hat{G}, \hat{a}]] = -\xi(-\xi^*\hat{a}) = |\xi|^2\hat{a} = r^2\hat{a}
\end{equation*}

\textbf{Higher-Order Commutators:}
The pattern predictably scales by $r^2$ with each cycle:
\begin{align*}
...
\subsection*{4. Summing the BCH Series}
Substituting these evaluated commutators back into the original expansion:
\begin{equation*}
...
\end{equation*}
Collecting terms containing $\hat{a}$ and $\hat{a}^\dagger$:
\begin{equation*}
...
\end{equation*}
Substituting $\xi = r e^{i\theta}$ and factoring out $r$ for the second series:
\begin{equation*}
...
\end{equation*}
These are the Taylor series for $\cosh(r)$ and $\sinh(r)$:
\begin{equation*}
\hat{S}^\dagger(\xi) \hat{a} \hat{S}(\xi) = \cosh(r)\hat{a} + e^{i\theta}\sinh(r)\hat{a}^\dagger
\end{equation*}

\subsection*{5. Final Expression}
...
\subsection*{1. Constructing the Normal-Ordered Ansatz}
...
\subsection*{2. Pushing the Creation Operator Through the Ansatz}
...
\subsection*{3. Pushing the Annihilation Operator Through the Ansatz}
...
\subsection*{4. Determining the Normalization Scalar}
...
\subsection*{6. Squeezed state form}
...
\section*{Derivation of the Wigner Function for the Squeezed Vacuum State}
...
\subsection*{1. The Annihilation Condition}
...
\subsection*{2. Formulating $\hat{b}$ in Phase Space}
...
\subsection*{3. Solving the Position Wavefunction}
...
\subsection*{4. Integrating the Wigner Function}
...
\section*{Algebraic Derivation of Two-Mode Squeezing}
...
\subsection*{3. Normal-Ordered Ladder Expansion}
...
\section*{Preliminary Properties for Usage}
...
\subsection*{1. The First Moments (Means)}
We begin by proving that the expectation values of the fundamental mode operators are strictly zero.
\begin{align*}
...
\end{equation*}
By applying the exact same logic and taking the complex conjugate, the creation operator mean is also zero:
\begin{equation*}
\langle \xi | \hat{a}^\dagger | \xi \rangle = 0
\end{equation*}

\subsection*{2. The Photon Number}
Next, we prove that the expected photon number $\langle\hat{n}\rangle = \langle \hat{a}^\dagger \hat{a} \rangle$ equals $\sinh^2 r$. To do this, we insert the identity $\hat{I} = \hat{S}\hat{S}^\dagger$ between the operators.
\begin{align*}
...
\end{align*}
We explicitly expand this binomial product, ensuring no terms are skipped:
\begin{equation*}
= \langle 0 | \mu^2 \hat{a}^\dagger\hat{a} + \mu\nu \hat{a}^\dagger\hat{a}^\dagger + \nu^*\mu \hat{a}\hat{a} + |\nu|^2 \hat{a}\hat{a}^\dagger | 0 \rangle
\end{equation*}
We now evaluate the action of these four operators on the vacuum state $|0\rangle$:
\begin{enumerate}
...
\end{enumerate}
Substituting these evaluations back into our expansion yields:
\begin{equation*}
...
\subsection*{3. The Variance / Squeezing Terms}
We now calculate the expectations of the paired annihilation and creation operators.
\begin{align*}
...
\end{align*}
Expanding the binomial product completely:
\begin{equation*}
= \langle 0 | \mu^2 \hat{a}\hat{a} + \mu\nu \hat{a}\hat{a}^\dagger + \nu\mu \hat{a}^\dagger\hat{a} + \nu^2 \hat{a}^\dagger\hat{a}^\dagger | 0 \rangle
\end{equation*}
Using the exact same vacuum evaluation rules established in Section 2, only the $\hat{a}\hat{a}^\dagger$ operator survives acting on $|0\rangle$:
\begin{equation*}
\langle \xi | \hat{a}\hat{a} | \xi \rangle = \mu\nu \langle 0 | \hat{a}\hat{a}^\dagger | 0 \rangle = \mu\nu
\end{equation*}
Substituting our definitions for $\mu$ and $\nu$:
\begin{equation*}
\langle \xi | \hat{a}\hat{a} | \xi \rangle = (\cosh r)(e^{i\theta}\sinh r) = e^{i\theta}\sinh r \cosh r
\end{equation*}

By applying identical logic to the creation operators, we evaluate $\langle \hat{a}^\dagger\hat{a}^\dagger \rangle$:
\begin{align*}
...
&= \mu\nu^* \langle 0 | \hat{a}\hat{a}^\dagger | 0 \rangle = \mu\nu^*
\end{align*}
Substituting the definitions yields the final statistic:
\begin{equation*}
...
\section*{Explicit Derivation of the Matrices for the Gaussian EPR State}
...
\subsection*{1. Deriving the Input Covariance Matrix ($\sigma_{in}$)}
...
\subsection*{2. Deriving the Symplectic Beam-Splitter Matrix ($B$)}
...
\subsection*{3. Deriving the Output EPR Covariance Matrix ($\sigma_{EPR}$)}
The final covariance matrix evolves under the symplectic transformation as $\sigma_{EPR} = B \sigma_{in} B^T$. We evaluate this via rigorous matrix multiplication. 
...
Populating the full array with these evaluated elements yields the exact output covariance matrix:
\begin{equation*}
